...
def endpoint_detection(signal, frame_length, hop_length, energy_threshold, zcr_threshold):
    ...
    current_start = start
    while current_start > 0 and (energy[current_start - 1] > energy_threshold or
                                 (zcr[current_start - 1] > zcr_hold and
                                  energy[current_start - 1] > 0.2 * energy_threshold)):
        current_start -= 1

    # 2. 扩展 end 指针
    current_end = end
    while current_end < energy_size - 1 and (energy[current_end + 1] > energy_threshold or
                                             (zcr[current_end + 1] > zcr_hold and
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".wav"):
            ...
            try:
                ...
                data = data.reshape(-1)  # 确保为单声道

                # 处理音频信号
                processed_data = del_signal_ini(sr, data)

                # 保存处理后的音频
                sf.write(output_path, processed_data, sr, subtype='PCM_24')

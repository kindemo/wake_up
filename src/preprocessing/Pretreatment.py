...
def split_audio_windows(wave, frame_length=400, num_win=33):
    ...
    :param n_mfcc:
    :param num_win:
    :return: 返回形状为 (num_bif_windows, big_frame_length) 的张量。
    """
    # 确保输入类型
    ...
    # 进行无重叠拼帧
    windows = get_windows(data, frame_length, num_windows=num_win)
...
def loading_file2windows(file_path, frame_length=400, num_win=33):
    ...
    返回形状为 (num_big_windows, big_frame_length) 的张量。
    """
    # 对tensorflow解码
    ...
    wave, s_rate = tf.audio.decode_wav(audio_binary,
                                       desired_channels=1)
    wave = squeezing(wave)
...
def load_and_split_audio(file_path: str, label: int, frame_length=400, n_mfcc=13, num_win=33):
    ...
    def py_load_and_split_audio(file_path_str, label_py):
        windows = loading_file2windows(file_path_str, frame_length, num_win)
        num_big_windows = windows.shape[0]
        labels = tf.repeat(label_py, num_big_windows)
...
class TestLoadAndSplitAudio(unittest.TestCase):
    @patch(__name__ + '.split_audio_windows')
    def test_load_and_split_audio(self, mock_split_audio_windows):
        # 模拟 split_audio_channels 函数的返回值
        mock_channels = tf.random.normal([32, 76, 13])
        mock_split_audio_windows.return_value = mock_channels
        ...
        self.assertEqual(labels.shape[0], windows.shape[0])
